...
@Controller
public class TotalSheetController extends BaseController {


    private static Logger logger = LoggerFactory.getLogger(TotalSheetController.class);

    @Autowired
    SettlementService settlementService;
    @Autowired
    TotalSheetService totalSheetService;
    @Autowired
    ...
    @ResponseBody
    @RequestMapping("salaryTotalSheet.do")
    private ModelAndView showPageList(String id) {
        // 跳转到用户管理页面
        ModelAndView mv = new ModelAndView();
        mv.setViewName("/hrms/salaryTotalSheet");
     ...
     */
    @ResponseBody
    @RequestMapping("ajax/total_getDatas.do")
    private void getDatas(SettlementSearchFilter filter) throws BusinessException {
        try {
            ...
            List<TotalSheetVo> userVos=null;
            int count = 0;
            HashMap<String, Object> hashmap = new HashMap<String, Object>();
            //设置权限
            if(getSessionUser().getDah().equals(ConstantKey.ADMIN))
            {
                userVos = totalSheetService.getDatas(filter);
...
//        }
    ...
    // getTotalSheetByMonthDept
    /**
     * @param filter
     * @throws BusinessException
     */
    @ResponseBody
    @RequestMapping("ajax/monthDept_getDatas.do")
    private void getTotalSheetByMonthDept(SettlementSearchFilter filter) throws BusinessException {
        try {
            // 用户查询条件
            filter.setOffset();
            filter.setLimit();
            ...
            if(getSessionUser().getDah().equals(ConstantKey.ADMIN))
            {
                userVos = totalSheetService.getTotalSheetByMonthDept(filter);
//                count = settlementService.getSettlementCount(filter);
                hashmap.put(ConstantKey.EDIT_FLAG, true);
                hashmap.put(ConstantKey.SEARCH_FLAG,true);
