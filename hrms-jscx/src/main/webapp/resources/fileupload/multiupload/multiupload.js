...
mini.extend(UserControl.MultiUpload, mini.DataGrid, {
    ...
    _create: function () {
        ...
        var defaultColumns = [
                    ...
                    { field: "complete", width: 60, header: me.columnsTexts.completeColumnHeader, headerAlign: "center" },
                    { field: "status", width: 60, header: me.columnsTexts.statusColumnHeader, align: "center", headerAlign: "center" },
                    { field: "action", width: 60, header: me.columnsTexts.actionColumnHeader, align: "center", headerAlign: "center" }
                  ];
        ...
        var sb = [];
        sb.push('<table><tr><td style="width:80px;height:25px;"><a class="mini-button" name="previewArea" iconCls="icon-search" style="width:80px">浏览...</a><span class="mini-upload"><span id="' + me._uploadId + '" class="mini-upload-placeholder" style=""></span></span>');
    ...
    bindEvents: function () {
        ...
        me.on("drawcell", function (e) {
            ...
            if (field == "action") {
                var htmlAction = '<a class="upicon-search" name="' + uid + '"><a>';
                htmlAction = htmlAction + '<a class="upicon-download" name="' + uid + '"><a>';
                htmlAction = htmlAction + '<a class="upicon-remove" name="' + uid + '"><a>';
                e.cellHtml = htmlAction;
    ...
    __on_file_queued_error: function (file, errorCode, msg) {
        if(errorCode == -110){
            mini.alert("文件大小超出30M");
        }else if(errorCode == -100){
            mini.alert("文件数量超出50个");
        }else{
